...
```js
<script>
import { MyMixin, MyConstants, MyUtil } from '@iy4u/common-client-lib'
...
```js
import '@iy4u/common-client-lib/assets/main.css'
```
...
In your client app, you don't need to explicitly add dependency to `moment` in `package.json` as it is a dependency of `@iy4u/common-client-lib`. However, if you want to reduce the size of the bundle size of client app, add the following in the `vue.config.js` of client app ([details](https://github.com/jmblog/how-to-optimize-momentjs-with-webpack)), assuming it is also built with Vue CLI .
...
<html>
  <head>
    ...
    <link rel="stylesheet" href="dist/common-client-lib.css">
  </head>
  <body>
    <div id="app">
  ...
  <script src="dist/common-client-lib.umd.js"></script>
  <script>
      console.log(window['common-client-lib'])
      var app = new Vue({
        el: '#app',
        methods: {
          onClick (message) {
