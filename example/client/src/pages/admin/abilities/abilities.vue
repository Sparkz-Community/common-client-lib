<template>
  <q-page id="abilities" class="q-pa-md">
    <q-table title="Abilities"
             :data="abilities"
             :columns="columns"
             row-key="_id"
             selection="multiple"
             ...
             grid-header
             dense
             :loading="isFindAbilitiesPending"
             color="accent"
      ...
      <template v-slot:top-right>
        ...
        <add-or-update-ability ref="addOrUpdate"
                            :value="formData"
                            class="q-mr-lg"
                            @saved="abilitiesRefresh"></add-or-update-ability>
                          ...
                          <q-item v-for="contact in col.value" :key="contact.id" class="q-mb-sm" clickable v-ripple>
                            <q-item-section avatar>
                              ...
                              <q-avatar color="primary" text-color="white">
                                {{ $lget(contact, '_fastjoin.ability.name', 'Unknown').substring(0, 1) }}
                              </q-avatar>
                            </q-item-section>

                            <q-item-section>
                              <q-item-label>{{ $lget(contact, '_fastjoin.ability.name', 'Unknown') }}</q-item-label>
                              <q-item-label caption lines="1">
                                {{ $lget(contact, '_fastjoin.ability.email', 'Unknown') }}
                              </q-item-label>
                            </q-item-section>
...
<script>
  ...
  import addOrUpdateAbility from 'pages/admin/abilities/addOrUpdateAbility/addOrUpdateAbility';
  ...
  export default {
    ...
    data() {
      let columns = [
        ...
        {
          name: '_fastjoin.rules',
          label: 'Rules',
          align: 'left',
          field: row => this.$lget(row, '_fastjoin.rules', []),
          format: val => val.map(rule => rule.name).join(', '),
          sortable: true,
        },
        {
          name: 'active',
      ...
      return {
        ...
        pagination: {
          ...
          page: 1,
          rowsPerPage: 12,
          rowsNumber: this.abilitiesTotal,
        },
        expandMeta: {},
        search: '',
        selected: [],
    ...
    watch: {
      ...
      //   immediate: true,
      //   handler(newVal) {
      //     this.abilitiesLimit = newVal.rowsPerPage;
      //     this.abilitiesCurrentPage = newVal.page;
      //   }
      // },
      abilitiesTotal(newVal) {
    ...
    methods: {
      ...
      setPagination(newVal) {
        ...
        this.abilitiesLimit = newVal.pagination.rowsPerPage === 0 ? this.abilitiesTotal : newVal.pagination.rowsPerPage;
        this.abilitiesCurrentPage = newVal.pagination.page;
      ...
      editDialog(props) {
        ...
        this.$lset(this.formData, 'ability', props.row);
