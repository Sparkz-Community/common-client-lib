  ...
  <q-page id="payments" class="q-pa-md">
    <q-table title="Payments"
      ...
      <template v-slot:top-right>
        ...
        <add-or-update-payment ref="addOrUpdate"
                               :value="formData"
                               class="q-mr-lg"
                               @saved="paymentsRefresh"></add-or-update-payment>
                          ...
                          <q-item v-for="contact in col.value" :key="contact.id" class="q-mb-sm" clickable v-ripple>
                            <q-item-section avatar>
                              ...
                              <q-avatar color="primary" text-color="white">
                                {{ $lget(contact, '_fastjoin.payment.name', 'Unknown').substring(0, 1) }}
                              </q-avatar>
                            </q-item-section>

                            <q-item-section>
                              <q-item-label>{{ $lget(contact, '_fastjoin.payment.name', 'Unknown') }}</q-item-label>
                              <q-item-label caption lines="1">
                                {{ $lget(contact, '_fastjoin.payment.email', 'Unknown') }}
                              </q-item-label>
                            </q-item-section>
...
<script>
  ...
  import addOrUpdatePayment from 'pages/admin/payments/addOrUpdatePayment/addOrUpdatePayment';
  ...
  export default {
      ...
      makeFindPaginateMixin({
        ...
        query() {
          return {
            $or: [
              {
                memo: {$regex: `${this.search}`, $options: 'igm'},
              },
            ],
            $sort: this.sort,
          };
    ...
    data() {
      let columns = [
        ...
        {
          name: 'transferredToExternal',
          label: 'Transferred to External',
          align: 'left',
          field: row => row.transferredToExternal,
          format: val => `${val}`,
          sortable: true,
          required: true,
        },
        {
          name: 'transferredToExternalDate',
          label: 'Transferred to External Date',
          align: 'left',
          field: row => row.transferredToExternalDate,
          format: val => `${val}`,
          sortable: true,
          required: true,
        },
        ...
        {
          name: 'receivedFromExternal',
          label: 'Received from External',
          align: 'left',
          field: row => row.receivedFromExternal,
          format: val => `${val}`,
          sortable: true,
          required: true,
        },
        {
          name: 'receivedFromExternalDate',
          label: 'Received from External Date',
          align: 'left',
          field: row => row.receivedFromExternalDate,
          format: val => `${val}`,
          sortable: true,
          required: true,
        },
    ...
    watch: {
      ...
      //   immediate: true,
      //   handler(newVal) {
      //     this.paymentsLimit = newVal.rowsPerPage;
      //     this.paymentsCurrentPage = newVal.page;
      //   }
      // },
      paymentsTotal(newVal) {
    ...
    methods: {
      ...
      setPagination(newVal) {
        ...
        this.paymentsLimit = newVal.pagination.rowsPerPage === 0 ? this.paymentsTotal : newVal.pagination.rowsPerPage;
        this.paymentsCurrentPage = newVal.pagination.page;
      ...
      editDialog(props) {
        ...
        this.$lset(this.formData, 'payment', props.row);
        this.formData = this.$lcloneDeep(this.formData);
      },
      removePayment(props) {
        this.$q.dialog({
          title: 'Confirm',
          message: `Are you sure you want to remove "${props.row.memo}"?`,
          ok: {
            push: true,
            color: 'negative',
          },
          ...
          props.row.remove({disableSoftDelete: true})
            .then(result => {
              this.$q.notify({
                type: 'positive',
                message: `Successfully removed "${result.memo}"`,
                timeout: 10000,
                actions: [
                  {
