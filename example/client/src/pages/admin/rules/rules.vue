  ...
  <q-page id="rules" class="q-pa-md">
    <q-table title="Rules"
      ...
      <template v-slot:top-right>
        ...
        <add-or-update-rule ref="addOrUpdate"
                            :value="formData"
                            class="q-mr-lg"
                            @saved="rulesRefresh"></add-or-update-rule>
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td v-for="col in props.cols"
                :key="col.name"
                :props="props">
            <span v-if="['conditions'].includes(col.name) && Object.keys(col.value).length">
              <q-btn size="xs"
                     color="secondary"
                     round
                     @click="setExpanded(props, col.name)"
                     :icon="props.expand && $lget(expandMeta,`${props.pageIndex}.expandKey`) === col.name ? 'remove' : 'add'">
                <q-badge color="accent" text-color="black" :label="Object.keys(col.value).length" floating transparent
                         rounded/>
              </q-btn>
            </span>
        ...
        <q-tr v-show="props.expand" :props="props">
          <q-td colspan="100%">
            <div v-if="['conditions'].includes($lget(expandMeta,`${props.pageIndex}.expandKey`))"
          ...
          <q-card :class="props.selected ? 'bg-grey-2' : ''">
            ...
            <q-list dense>
              <div v-for="col in props.cols" :key="col.name">
                <q-btn v-if="['conditions'].includes(col.name)"
                       align="left"
                       class="full-width"
                       flat
                       @click="setExpanded(props, col.name)">
                  ...
                  <q-dialog v-if="['conditions'].includes($lget(expandMeta,`${props.pageIndex}.expandKey`))"
                          ...
                          <q-item v-for="contact in col.value" :key="contact.id" class="q-mb-sm" clickable v-ripple>
                            <q-item-section avatar>
                              ...
                              <q-avatar color="primary" text-color="white">
                                {{ $lget(contact, '_fastjoin.rule.name', 'Unknown').substring(0, 1) }}
                              </q-avatar>
                            </q-item-section>

                            <q-item-section>
                              <q-item-label>{{ $lget(contact, '_fastjoin.rule.name', 'Unknown') }}</q-item-label>
                              <q-item-label caption lines="1">
                                {{ $lget(contact, '_fastjoin.rule.email', 'Unknown') }}
                              </q-item-label>
                            </q-item-section>
...
<script>
  ...
  import addOrUpdateRule from 'pages/admin/rules/addOrUpdateRule/addOrUpdateRule';
  ...
  export default {
    ...
    watch: {
      ...
      //   immediate: true,
      //   handler(newVal) {
      //     this.rulesLimit = newVal.rowsPerPage;
      //     this.rulesCurrentPage = newVal.page;
      //   }
      // },
      rulesTotal(newVal) {
    ...
    methods: {
      ...
      setPagination(newVal) {
        ...
        this.rulesLimit = newVal.pagination.rowsPerPage === 0 ? this.rulesTotal : newVal.pagination.rowsPerPage;
        this.rulesCurrentPage = newVal.pagination.page;
      ...
      editDialog(props) {
        ...
        this.$lset(this.formData, 'rule', props.row);
