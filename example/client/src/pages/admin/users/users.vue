  ...
  <q-page id="users" class="q-pa-md">
    <q-table title="Users"
      ...
      <template v-slot:top-right>
        ...
        <add-or-update-user ref="addOrUpdate"
                            :value="formData"
                            class="q-mr-lg"
                            @saved="usersRefresh">
                          ...
                          <q-item v-for="contact in col.value" :key="contact.id" class="q-mb-sm" clickable v-ripple>
                            <q-item-section avatar>
                              ...
                              <q-avatar color="primary" text-color="white">
                                {{ $lget(contact, '_fastjoin.user.name', 'Unknown').substring(0, 1) }}
                              </q-avatar>
                            </q-item-section>

                            <q-item-section>
                              <q-item-label>{{ $lget(contact, '_fastjoin.user.name', 'Unknown') }}</q-item-label>
                              <q-item-label caption lines="1">
                                {{ $lget(contact, '_fastjoin.user.email', 'Unknown') }}
                              </q-item-label>
                            </q-item-section>
...
<script>
  ...
  import addOrUpdateUser from 'pages/admin/users/addOrUpdateUser/addOrUpdateUser';
  ...
  export default {
    ...
    data() {
      let columns = [
        ...
        {
          name: '_fastjoin.roles',
          label: 'Roles',
          align: 'left',
          field: row => this.$lget(row, '_fastjoin.roles', []),
          format: val => `${val.map(role => role.name).join(', ')}`,
          sortable: true,
        },
        {
          name: 'isVerified',
    ...
    watch: {
      ...
      //   immediate: true,
      //   handler(newVal) {
      //     this.usersLimit = newVal.rowsPerPage;
      //     this.usersCurrentPage = newVal.page;
      //   }
      // },
      usersTotal(newVal) {
    ...
    methods: {
      ...
      setPagination(newVal) {
        ...
        this.usersLimit = newVal.pagination.rowsPerPage === 0 ? this.usersTotal : newVal.pagination.rowsPerPage;
        this.usersCurrentPage = newVal.pagination.page;
      ...
      editDialog(props) {
        ...
        this.$lset(this.formData, 'user', props.row);
