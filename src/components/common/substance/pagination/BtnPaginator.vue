<template>
  <div style="display: flex">
    <q-btn size="sm" v-if="tab - 1 > 0" flat @click="setTab(1)" :label="1"/>
    <q-btn size="sm" flat v-if="tab > 0" @click="tab > 0 ? setTab(tab) : undefined" label="..."/>
    <div v-for="(page, i) in showPages.list"
         :key="`page-${i}`"
         v-show="shouldItShow(page)">
      <q-btn :class="tab === (i) ? `bg-${color} text-dark` : ''"
             size="sm"
             class="pagination"
             @click="setTab(page)"
             flat
    ...
    <q-btn flat v-if="pages - 1 > tab" @click="(tab + 1) < pages ? setTab(tab + 2) : ''" label="..."/>
    <q-btn dense v-if="tab + 2 < pages" flat @click="setTab(pages)" :label="pages"/>
